...
def highest_label_preflow_push_algo(oriG):
    ...
    while active_nodes:
        while not active_LIST[highest_label]:
            highest_label = highest_label - 1
        i = active_LIST[highest_label].pop()
        ...
        while excess(i) and admissible_arcs:
            ...
            if v not in active_nodes and v not in [s, t]:
                ...
                highest_label = max(highest_label, oriG.node[v]['d'])
            if PLOT:
        ...
        if excess(i):
            ...
            if GAP and all(d(j) != g for j in oriG.node() if j != i):
                    ...
                    if g < d(j) < N or j == i:
                        ...
                        if FREEZE and excess(j):
                            ...
                            if j in active_nodes:
                                ...
                                active_LIST[pre_d].remove(j)
                        elif excess(j):
                            active_nodes.add(j)
                            active_LIST[d(j)].add(j)
                ...
                if FREEZE and d(i) >= N:
                    frozen_nodes.add(i)
                    print('freeze {}: {}'.format(i, d(i)))
                    if PLOT:
                        title = 'freeze {}: {}'.format(i, d(i))
                        draw_graph(oriG, active_nodes=active_nodes,
                            frozen_nodes=frozen_nodes, title=title, node_label='d')
                else:
                    active_nodes.add(i)
    ...
    if FREEZE and frozen_nodes:
        ...
        for frozen_node in sorted(frozen_nodes, key=sdepth, reverse=True):
            ...
            while excess_value:
                ...
                childs = [v for v in set(resG[tail_node]) - visited_nodes
                    if tail_node in oriG[v] and oriG[v][tail_node]['preflow'] > 0]
                if not childs:
                    path.pop()
...
def dfs_set_label(oriG, resG, active_nodes):
    active_LIST = {i: set() for i in range(2*len(oriG.nodes()))}
    highest_label = 0
    labeld_nodes = {s, t}
    depth = 0
            ...
            for v in resG[u]:
                if v not in labeld_nodes:
                    ...
                    if v in active_nodes:
                        active_LIST[depth].add(v)
                        highest_label = depth
                    labeld_nodes.add(v)
                    next_stack.append(v)
        stack = next_stack
        next_stack = []
    return active_LIST, highest_label
